...
% % condition 1, filting the highest point 2, get the No of point 
% 
% O23_con1=find(O2_d(:,3)==min(O2_d(:,3)));
% 
% % new O2 O3
% O3_conp=O3_d(O23_con1,:);
% O2_conp=O2_d(O23_con1,:);
% 
% % the distance from the point o2 to z_axis
% Lo2_con2=(O2_conp(:,1).^2+O2_conp(:,2).^2).^0.5;
% 
% % condition 2, shortest o2d distance, and get the No
% O23_con2=find(Lo2_con2==max(Lo2_con2));
% 
% % get the new 2 and 3
% O3_conp2=O3_conp(O23_con2,:);
% O2_conp2=O2_conp(O23_con2,:);
% 
% % the distance from the point o3 to z_axis
% Lo3_con3=(O3_conp2(:,1).^2+O3_conp2(:,2).^2).^0.5;
% 
% % condition 3, shortest distance and get the No
% O23_con3=find(Lo3_con3==min(Lo3_con3));
% 
% % the final point getten 
% O2_fp=O2_conp2(O23_con3(1),:);
% O3_fp=O3_conp2(O23_con3(1),:);


% plot3(O3_d(:,1),O3_d(:,2),O3_d(:,3),'*')
% plot3(O2_d(:,1),O2_d(:,2),O2_d(:,3),'o')
O0_pick=O0;
O4_pick=O4;
O1_pick=O1;
O2_d_pick=O2_d;
...
point_joint_pick1=[O0_pick;O1_pick;O4_pick];
point_joint_pick2=[O2_d_pick;O3_d_pick];

save('point_joint_pick1','O0_pick','O1_pick','O4_pick')
save('point_joint_pick2','O2_d_pick','O3_d_pick')
tf0=1;
%  plot3(point_joint(:,1),point_joint(:,2),point_joint(:,3))
%  xlim([-0.6 0.6])
%  ylim([-0.6 0.6])
%  zlim([-0.6 0.6])